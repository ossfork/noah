...
fn default_app_dir() -> Result<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        Ok(PathBuf::from(home).join("Library/Application Support/app.onnoah.tinkerers"))
    }
    #[cfg(target_os = "linux")]
    {
        Ok(PathBuf::from(home).join(".local/share/app.onnoah.tinkerers"))
    }
    #[cfg(target_os = "windows")]
    {
        Ok(PathBuf::from(home).join("AppData/Roaming/app.onnoah.tinkerers"))
    }
}
